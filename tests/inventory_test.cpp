...
//     EXPECT_EQ(model->get_commodities().size(), 1);
//     EXPECT_EQ(model->get_modules().size(), 1);
//     delete model;
// }
...
TEST(InventoryTest, PutTwoCommodity){
    ...
    EXPECT_EQ(model->get_commodities().size(), 0);
    model->update_commodity((*all_commodities.get())[0], 2);
    model->update_commodity((*all_commodities.get())[1], 2);
    EXPECT_EQ(model->get_commodities().size(), 2);
    delete model;
}
...
TEST(InventoryTest, PutTwoModule){
    ...
    EXPECT_EQ(model->get_modules().size(), 0);
    model->update_module((*all_modules.get())[0], 2);
    model->update_module((*all_modules.get())[1], 2);
    EXPECT_EQ(model->get_modules().size(), 2);
    delete model;
}